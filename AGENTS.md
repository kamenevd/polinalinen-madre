...
| Signed APK текущий | `/opt/madre-releases/madre-v6.6.5.apk` (+ `-manifest.json`) |
...
Стоп: **6.6.5 (36)** в модерации RuStore. Второй бинарь не собирать и не отправлять, пока эта версия не покинет очередь. Публичная версия до одобрения — 6.4.5 (33).

Doctor 6.6.5 подтвердил дефект следующего патча: без доступной полки скрытый дефолт `PUT_WITH_PHOTO` может запросить фото или поставить shelf sync в очередь без видимого выбора. Не лечить текущий pending-бинарник; исправить и покрыть полным тестом в следующем patch cycle.
