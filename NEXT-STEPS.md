# NEXT-STEPS — указатель на текущий цикл

С 25.07.2026 этот файл — указатель, не очередь.

Источники:
- `workflow/CYCLE.yaml` — versioned manifest
- `docs/WORKFLOW-V2.md` — правила
- `python3 scripts/cycle.py validate` — контракт

**Текущий:** **maintenance/20** — ревизия таймеров и уведомлений
(`workflow/CYCLE.yaml`, `DESIGN-V4.md` → Cycle 20). Версия останется за
prepare-version, руками не бампается.

Предыдущий: Cycle 19 shipped as **v6.3.1-cycle19** (versionCode 27).

APK: https://kdnfx.space/downloads/madre-v6.3.1-cycle19.apk  
Release: https://github.com/kamenevd/polinalinen-madre/releases/tag/v6.3.1-cycle19

Невосстановимые v3 №2/4/5 — `docs/adr/0002-unverifiable-v3-bug-list.md`.
Evidence gap 15/16 — `docs/adr/0003-missing-cycle-evidence.md`.
