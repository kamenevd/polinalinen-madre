...
/**
 * Cycle 20: шаг тика таймера — и то, сколько спать до следующего.
 *
 * Ровная секунда промахивается мимо конца шага: тики идут от момента, когда
 * человек нажал «Начать», а шаг кончается когда кончается, и между этими двумя
 * сетками до секунды разницы. Всё это время страница показывает 0:01 у уже
 * закончившегося шага, а «время вышло» приходит с опозданием.
 *
 * Отдельным объектом, а не парой строк внутри цикла: внутри корутины эту
 * арифметику проверить нечем, а промах здесь виден человеку каждым шагом.
 */
object BakingTick {

    /** Обычный шаг тика: цифры на странице меняются раз в секунду. */
    const val INTERVAL_MS = 1_000L

    /**
     * Сколько спать до следующего тика. Пока до конца шага меньше секунды —
     * спим ровно до него, чтобы проснуться В ноль. После границы (и когда её
     * вовсе нет) — обычная секунда: шаг досчитан, но страница живёт дальше,
     * пока человек не нажмёт «Дальше», и крутить цикл впустую незачем.
     */
    fun sleepMillis(nowElapsed: Long, stepEndsAtElapsed: Long): Long {
        val untilEnd = stepEndsAtElapsed - nowElapsed
        return if (untilEnd in 1..INTERVAL_MS) untilEnd else INTERVAL_MS
    }
}
...
data class BakingSession(
    ...
    /**
     * Cycle 20: момент, когда текущий шаг досчитает до нуля, — по тем же
     * монотонным часам, что и весь отсчёт.
     *
     * Нужен ровно двоим: системному хронометру в шторке (ему нужна база, а не
     * остаток) и тику таймера, который обязан попасть В эту границу, а не мимо
     * неё. До этого цикла база считалась как «сейчас плюс остаток в секундах»,
     * а остаток округлён вниз: шаг, начавшийся в .400, кончается в .400, и
     * хронометр, поставленный по круглой секунде, добегал до нуля позже самого
     * шага. Здесь конец — свойство шага, а не момента вопроса: пока отсчёт
     * идёт, [nowElapsed] на ответ не влияет вовсе.
     *
     * На паузе — иначе: конец уезжает вместе с паузой, и «через сколько» имеет
     * смысл только от «сейчас». Хронометром это всё равно не рисуется
     * (остановленный Chronometer врал бы), но число обязано быть честным.
     */
    fun stepEndsAtElapsed(nowElapsed: Long): Long {
        val durationMillis = currentStep.durationMinutes * 60_000L
        if (!isPaused) return startedAtElapsed + durationMillis
        val consumed = ((pausedAtElapsed ?: nowElapsed) - startedAtElapsed).coerceAtLeast(0L)
        return nowElapsed + (durationMillis - consumed).coerceAtLeast(0L)
    }
