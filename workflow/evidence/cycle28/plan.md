...
**R15–R19** (П17.11) и четырьмя строками редакции 6 — **R20–R23**: две живые
...
**Редакция 6 правит этот абзац в двух местах.** Применений superuser-токена
теперь **два**, и оба перечислены: пишущее — та же фикстура R18, читающее —
аудит журнала `family_members` (только id, статусы, число строк), без которого
R19 остаётся пересказом. Правила и границы этого чтения — Р6.7; ни одно
утверждение о доступе на нём по-прежнему не строится. И матрица прогона
удлиняется: строк теперь **R1–R23** — добавлены R20 и R21 (одновременный старт,
Р6.6), R22 (сбой счёта участников на копии хука с инъекцией дефекта, Р6.1) и
R23 (обе величины `status` серверными путями, Р6.2). R22 выполняется последним и
несёт свой отдельный провенанс подмены и возврата хука.
...
| «Единственное законное применение superuser-токена в этом прогоне — подготовка фикстуры R18» (раздел про провенанс) | **Р6.7**: применений два, оба перечислены; второе — аудируемое чтение журнала, и ни одно утверждение о доступе на него не опирается |
...
Ожидание: ни одно из шести обращений не возвращает байтов кадра. Точный код
(`403` или `404`) не предсказываем — записываем, что пришло; `200` с
изображением — release blocker, а не наблюдение. Плюс `POST /api/files/token`
постороннего: статус и факт выдачи записываются тоже — «токен ему выдали, и он
всё равно ничего не открыл» и есть содержательный результат этой строки.
...
2. **Что нельзя.** Запись — кроме единственной названной фикстуры R18
 (удаление строки, чтобы смоделировать уход до деплоя) и её отката. И ни одно
 утверждение о **доступе** на superuser не опирается: R6–R10, R15–R18,
 R20–R23 остаются обычными пользовательскими токенами. «Superuser увидел
 строку» никогда не значит «человеку дали доступ» — правило
 `family = @request.auth.family` для superuser не действует вовсе.
...
Это отменяет фразу редакции 5 «единственное законное применение
superuser-токена в этом прогоне — подготовка фикстуры R18»: применений два,
одно пишущее (R18) и одно читающее (R19, R20, R21, R23.1, R23.4), и оба
перечислены поимённо. Ограничение «ACL проверяется только обычными токенами»
